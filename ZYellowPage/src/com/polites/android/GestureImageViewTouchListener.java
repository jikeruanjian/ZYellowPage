...
public class GestureImageViewTouchListener implements OnTouchListener {
	...
	protected void calculateBoundaries() {
		...
		int effectiveWidth = Math.round( imageWidth * currentScale );
		int effectiveHeight = Math.round( imageHeight * currentScale );
		
		canDragX = effectiveWidth > displayWidth;
		canDragY = effectiveHeight > displayHeight;
		
		if(canDragX) {
			float diff = (effectiveWidth - displayWidth) / 2.0f;
			boundaryLeft = centerX - diff;
			boundaryRight = centerX + diff;
		}
		
		if(canDragY) {
			float diff = (effectiveHeight - displayHeight) / 2.0f;

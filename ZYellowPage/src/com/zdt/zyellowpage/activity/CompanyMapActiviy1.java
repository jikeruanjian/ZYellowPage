...
public class CompanyMapActiviy1 extends AbActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (getIntent().getExtras() != null) {
			userCompanyFullName = (String) getIntent().getExtras().get(
					"FUllNAME");
	...
	void initMapView() {
		...
		OverlayTest itemOverlay = new OverlayTest(getResources().getDrawable(
				R.drawable.companymap_mark_min), mMapView);
		itemOverlay.addItem(new OverlayItem(poiPoint, userCompanyFullName,
				userCompanyFullName));
	...
	void getNearCompanyData(int max, int distance) {
		...
		NearCompanyReqEntity nearCompanyReqEntity = new NearCompanyReqEntity(
				max, Double.valueOf(latitude), Double.valueOf(Longitude),
				distance);
					...
					public void onFinish() {
						...
						if (newList != null) {
							for (int j = 0; j < newList.size(); j++) {
								...
								if (user.getLatitude().toString()
										.equals(latitude)
										&& user.getLongitude().toString()
												.equals(Longitude))
									poiOverlayx.onTap(j);
							}
						}
	...
	public void getNearCompanyDataByType(String typeid) {
		UserBll bll = new UserBll();
		NearCompanyReqEntity nearCompanyReqEntity = new NearCompanyReqEntity(
				20, Double.valueOf(latitude), Double.valueOf(Longitude),
				poiDistance, typeid);
	...
	@SuppressLint("NewApi")
	void initMKSearch() {
		CompanyMapActiviy1.this.findViewById(R.id.btn_PioAllDistance)
					...
					public void onClick(View v) {
						...
						if (poiPoint != null) {
							...
							if (poiPoint != null) {
								...
								Toast.makeText(CompanyMapActiviy1.this,
										"请选择目的地！", Toast.LENGTH_LONG).show();
							}
						}
					}
		...
		// 初始化公交线路覆盖物
		TransitrouteOverlay = new TransitOverlay(CompanyMapActiviy1.this,
				mMapView);
	...
	class MyPoiOverlayX extends ItemizedOverlay {
		...
		protected boolean onTap(int i) {
			...
			if (info.getLogo() != null) {
				new AbImageDownloader(CompanyMapActiviy1.this).display(
						popImage, info.getLogo());
			}
	...
	public class MapMySearchListener implements MKSearchListener {
		...
		public void onGetTransitRouteResult(MKTransitRouteResult res, int error) {
			...
			if (error != 0 || res == null) {
				Toast.makeText(CompanyMapActiviy1.this, "抱歉，未找到结果",
						Toast.LENGTH_SHORT).show();
				return;
			}

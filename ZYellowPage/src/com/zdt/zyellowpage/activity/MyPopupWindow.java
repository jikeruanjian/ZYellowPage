...
public class MyPopupWindow {
	...
	public MyPopupWindow(Context context) {
		...
		mContext = context;
		layoutLeft = (LinearLayout) layout
				.findViewById(R.id.LeftLinearLayoutClass);
		layoutBClass = (LinearLayout) layout
				.findViewById(R.id.leftLayoutBussniessClass);
		layoutPClass = (LinearLayout) layout
				.findViewById(R.id.leftLayoutPersonClass);
		layoutRight = (LinearLayout) layout
				.findViewById(R.id.rightLinearLayoutClass);
		listViewClassB = (ListView) layout
				.findViewById(R.id.ListViewBussniessClass);
		listViewClassP = (ListView) layout
				.findViewById(R.id.ListViewPersonClass);
		listViewClassLower = (ListView) layout
				.findViewById(R.id.listViewLowerClass);
		tVBussniess = (TextView) layout
				.findViewById(R.id.textViewBussniessClass);
		...
		listViewClassLower.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if (layoutPClass.getVisibility() == View.GONE) {
					intent = new Intent(mContext, PopBusinessListActivity.class);
					intent.putExtra("Type", listLowerCategory.get(arg2)
							.getName());
					intent.putExtra("TypeId",
							"list-" + listLowerCategory.get(arg2).getId());
					mContext.startActivity(intent);
				} else {
					intent = new Intent(mContext, PopPersonListActivity.class);
					intent.putExtra("Type", listLowerCategory.get(arg2)
							.getName());
					intent.putExtra("TypeId",
							"list-" + listLowerCategory.get(arg2).getId());
	...
	private void changeClassType() {
		...
		if (layout.findViewById(R.id.leftLayoutPersonClass).getVisibility() == View.VISIBLE) {
			tVBussniess.setTextColor(mContext.getResources().getColor(
					R.color.orange));
			tVPerson.setTextColor(mContext.getResources().getColor(
					R.color.gray_black));
			tVPerson.setText("个人分类▼");
			tVBussniess.setText("商家分类▲");
			layout.findViewById(R.id.leftLayoutPersonClass).setVisibility(
					View.GONE);
			layout.findViewById(R.id.leftLayoutBussniessClass).setVisibility(
					View.VISIBLE);
			getRightData("0100", changeView);
			// listViewClassB.setSelection(0);
		} else {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				holder.title = (TextView) convertView
						.findViewById(R.id.textViewTopClass);
				holder.viewBtn = (TextView) convertView
						.findViewById(R.id.imageButtonLowerClass);
	...
	/*
	 * private class LeftClassBtnListener implements OnClickListener{ String
	 * oId; public LeftClassBtnListener(String id){ oId= id; }
	 * 
	 * @Override public void onClick(View v) {
	 * //((TextView)v).setTextColor(mContext
	 * .getResources().getColor(R.color.orange)); getRightData(oId,changeView);
	 * } }
	 */
	...
	void getRightData(String oId, View v) {
		...
		categoryBll.getCategoryist(mContext, oId, type,
